...
class APITestApp:
    ...
    def create_widgets(self):
        ...
        # Logo
        self.logo_label = ctk.CTkLabel(self.sidebar_frame, text="API Stress Test (C4S)", 
    ...
    def send_request(self, session, endpoint_info):
        ...
        try:
            ...
            if endpoint_info['type'] == 'form':
                ...
                # Modificación para enviar datos aleatorios en el body para Callforsupp
                if endpoint == "/Callforsupp/InitCallforsupp":
                    data_to_send = {
                        "callforsupp": "",
                        "idUser": random.choice([i for i in range(1, 51) if i != 16]),
                        "idType": random.randint(1, 3),
                        "message": "Texto",
                        "credibility": 1
                    }
                else:
